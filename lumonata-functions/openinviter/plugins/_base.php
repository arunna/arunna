...
abstract class openinviter_base
	...
	protected function startSession($session_id=false)
		...
		if ($session_id)
			{
			$path=getcwd().$this->getCookiePath($session_id);
	...
	protected function getCookiePath($session_id=false)
		...
		else $path=$path=$this->settings['cookie_path'].DIRECTORY_SEPARATOR.'oi.'.$this->getSessionID().'.cookie';
		return $path;
		}
	...
	protected function getLogoutPath($session_id=false)
		{
		if ($session_id) $path=$path=$this->settings['cookie_path'].DIRECTORY_SEPARATOR.'oi.'.$session_id.'.logout';
		else $path=$path=$this->settings['cookie_path'].DIRECTORY_SEPARATOR.'oi.'.$this->getSessionID().'.logout';
		return $path;
		}
	...
	protected function logAction($message,$type='error')
		...
		$log_file=fopen(getcwd().$log_path,'a');
