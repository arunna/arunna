 ...
 *
 * Added  prefix to avoid conflicts with existing kses users
 *
 * @version 0.2.2
...
/**
 * You can override in a plugin file.
 *
 * * 
 */
 ...
 */
function kses($string, $allowed_html, $allowed_protocols = array ('http', 'https', 'ftp', 'ftps', 'mailto', 'news', 'irc', 'gopher', 'nntp', 'feed', 'telnet')) {
	$string = kses_no_null($string);
	$string = kses_js_entities($string);
	$string = kses_normalize_entities($string);
	$allowed_html_fixed = kses_array_lc($allowed_html);
	//$string = kses_hook($string, $allowed_html_fixed, $allowed_protocols); // WP changed the order of these funcs and added args to kses_hook
...
function kses_split($string, $allowed_html, $allowed_protocols) {
	...
	return preg_replace_callback('%((<!--.*?(-->|$))|(<[^>]*(>|$)|>))%',
		create_function('$match', 'global $pass_allowed_html, $pass_allowed_protocols; return kses_split2($match[1], $pass_allowed_html, $pass_allowed_protocols);'), $string);
}
...
function kses_bad_protocol_once($string, $allowed_protocols) {
	...
	if ( isset($string2[1]) && !preg_match('%/\?%', $string2[0]) )
		$string = kses_bad_protocol_once2($string2[0]) . trim($string2[1]);
	else
		$string = preg_replace_callback('/^((&[^;]*;|[\sA-Za-z0-9])*)'.'(:|&#58;|&#[Xx]3[Aa];)\s*/', 'kses_bad_protocol_once2', $string);
...
function kses_normalize_entities($string) {
	...
	$string = preg_replace_callback('/&amp;#([Xx])0*(([0-9A-Fa-f]{2}){1,2});/', 'kses_normalize_entities3', $string);
 ...
 * @return string Filtered content
 */
function filter_kses($data) {
	global $allowedtags;
	return addslashes( kses(stripslashes( $data ), $allowedtags) );
}
 ...
 */
function filter_post_kses($data) {
	global $allowedposttags;
	return addslashes ( kses(stripslashes( $data ), $allowedposttags) );
}
...
function kses_post($data) {
	global $allowedposttags;
	return kses( $data , $allowedposttags );
}
...
function filter_nohtml_kses($data) {
	return addslashes ( kses(stripslashes( $data ), array()) );
}

/**
 * Adds all Kses input form content filters.
 *
 * All hooks have default priority. The filter_kses() function is added to
 * the 'pre_comment_content' and 'title_save_pre' hooks.
 *
 * The filter_post_kses() function is added to the 'content_save_pre',
...
function kses_init_filters() {
	// Normal filtering.
	add_filter('pre_comment_content', 'filter_kses');
	add_filter('title_save_pre', 'filter_kses');

	// Post filtering
	add_filter('content_save_pre', 'filter_post_kses');
	add_filter('excerpt_save_pre', 'filter_post_kses');
	add_filter('content_filtered_save_pre', 'filter_post_kses');
}
...
function kses_remove_filters() {
	// Normal filtering.
	remove_filter('pre_comment_content', 'filter_kses');
	remove_filter('title_save_pre', 'filter_kses');

	// Post filtering
	remove_filter('content_save_pre', 'filter_post_kses');
	remove_filter('excerpt_save_pre', 'filter_post_kses');
	remove_filter('content_filtered_save_pre', 'filter_post_kses');
}
